...
def loglikelihood(theta: np.ndarray, y: np.ndarray, x: np.ndarray) -> float:
    """The likelihood criterion function, returns the scalar sum of
    the log-likelihood values over all observations.
    ...
    Returns:
        float: Scalar value of the total log-likelihood.
    """
    # Unpack values
    N,K = x.shape
    ...
    residual = y - x @ beta
    ll_vec =  -0.5 * (np.log(2 * np.pi * sigma2) + (residual**2) / sigma2)
